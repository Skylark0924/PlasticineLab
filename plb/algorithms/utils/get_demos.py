...
def get_demo(env_name, path):
    ...
    if args.algo == 'sac':
        train_sac(env, path, logger, args)
    elif args.algo == 'action':
        solve_action(env, path, logger, args)
    elif args.algo == 'ppo':
        train_ppo(env, path, logger, args)
    elif args.algo == 'td3':
        train_td3(env, path, logger, args)
    elif args.algo == 'nn':
        solve_nn(env, path, logger, args)
    else:
