...
ENVS = []
for env_name in ['Move', 'Torus', 'Rope', 'Writer', "Pinch", "Rollingpin", "Chopsticks", "Table", 'TripleMove',
                 'Assembly', 'Doughstretch']:
    for id in range(5):
        register(
...
def make_new(env_name, learned_model, args, use_gpu, device):
    env: PlasticineEnv = gym.make(env_name, args=args, nn=False, learned_model=learned_model, use_gpu=use_gpu,
                                  device=device)
    return env
